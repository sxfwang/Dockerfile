#!/bin/bash
# shellcheck disable=SC1090,SC2005,SC2086,SC2155
...
INSTALL_APEX_ORDS_FILE="${INSTALL_APEX_ORDS_FILE:-installApexOrds.sh}"
START_ORDS_FILE="${START_ORDS_FILE:-startOrds.sh}"
ORDS_CONFIG="${ORDS_CONFIG:-$ORACLE_BASE/oradata/ords-config}"

########### Stop ORDS helper ############
function stopOrds {
   if [ -f /tmp/ords.pid ] && kill -0 "$(cat /tmp/ords.pid)" 2>/dev/null; then
      echo "Stopping ORDS."
      kill "$(cat /tmp/ords.pid)" 2>/dev/null
   fi;
}

########### Install APEX/ORDS helper ############
# Installs APEX into the PDB and configures ORDS when enabled. The install
# script itself is idempotent (checkpoint file in the persistent oradata
# volume), so it is safe to call it on every startup.
function installApexOrdsIfNeeded {
   if [ "${APEX_ORDS_ENABLED:-false}" = "true" ] && \
      [ "${STANDBY_DB:-false}" != "true" ] && \
      [ "${CLONE_DB:-false}" != "true" ] && \
      [ "${TRUE_CACHE:-false}" != "true" ]; then
      run_or_debug "installApexOrds.sh" "${SCRIPT_BASE_DIR}/$INSTALL_APEX_ORDS_FILE" || exit 1;
   fi;
}

########### Start ORDS helper ############
# Starts ORDS in the background once it has been configured.
function startOrdsIfNeeded {
   if [ "${APEX_ORDS_ENABLED:-false}" = "true" ] && [ -d "$ORDS_CONFIG/databases" ]; then
      "${SCRIPT_BASE_DIR}/$START_ORDS_FILE"
   fi;
}
...
if [ -f "$ORACLE_BASE"/oradata/.${ORACLE_SID}"${CHECKPOINT_FILE_EXTN}" ] && [ -d "$ORACLE_BASE"/oradata/"${ORACLE_SID}" ]; then
   ...
   # Install APEX/ORDS if enabled (covers oradata volumes created by an older
   # image without APEX/ORDS; the install script is idempotent)
   installApexOrdsIfNeeded;
  ...
  # Install APEX/ORDS into the freshly created database
  installApexOrdsIfNeeded;
...
if [ $status -eq 0 ]; then
  ...
  # Start ORDS if it has been configured and is enabled
  startOrdsIfNeeded;
